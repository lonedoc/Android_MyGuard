...
class PasswordApi(private val sessionDataHolder: SessionDataHolder) {
    ...
    suspend fun requestPassword(phoneNumber: String): Boolean {
        ...
        val query = getPasswordQuery(phoneNumber)
    ...
    private fun getPasswordQuery(phoneNumber: String) = JsonObject().run {
        addProperty("\$c$", "getpasswordlk")
        addProperty("phone", phoneNumber)
        toString()
    }
